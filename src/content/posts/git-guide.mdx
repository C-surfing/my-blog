---
title: "Git & GitHub 核心概念大串讲：从零到一全攻略"
...
> **本文基于技术爬爬虾的B站视频《Git+Github核心概念大串讲，从零到一全攻略，详细实战教程》整理补充而成。**
> 视频原文引用自 [BV1ySLc6QEcB](https://www.bilibili.com/video/BV1ySLc6QEcB)。

<div align="center">
<img src="/images/git-guide/cover.jpg" alt="Git & GitHub 视频封面" width="800" />
</div>
...
Git 与 GitHub 已经成为了 AI 时代必学必会的基本功之一。很多 AI Agent（如 Claude Code、Codex）的核心功能都是直接围绕着 Git 来运行的。

好消息是：在 AI 时代，我们不再需要死记硬背一大堆 Git 命令，只需要掌握 Git 和 GitHub 的几个核心概念，就可以直接用自然语言指挥 AI 完成各种操作。

本文的目标：**让你理解 Git 的核心原理，而不是记忆命令。**

> 正如庄子所言：「言者所以在意，得意而忘言」—— 在 AI 时代，我们只需要理解 Git 操作的本意与原理，具体的命令并不需要死记硬背。
...
Git 是一个 **开源免费** 的版本控制软件。想象一下写论文的场景：
...
这种"人工版本控制"在个人小项目中还能应付，但当一个项目有成千上万个文件、成百上千人协同开发时，就必须使用 Git 这样的专业工具。

Git 的核心能力：
- **快照式保存**：每次 commit（提交）都保存仓库当前状态的快照
- **完整可回溯**：形成一条 commit 历史链路，每个参与者的每次改动都被记录
- **分布式架构**：每个人的电脑上都有完整的仓库副本

### 2.2 GitHub — 代码的社交中心

Hub 的意思是「中心、集合」。GitHub 就是全球最大的 **Git 仓库托管与协作平台**，你可以在这里：
...
GitHub 上的仓库分两种：
- **公开仓库（Public）**：所有人都可以浏览、学习、使用
- **私有仓库（Private）**：只有你自己或你授权的协作者才能看到

> Linux 操作系统、Python 的 C 语言解释器（CPython）、Nginx 等知名项目都托管在 GitHub 上。开源是计算机科学的基石与瑰宝。
...
**Windows 用户：**
- 前往 [git-scm.com](https://git-scm.com) 下载 Windows 安装包
- 一路点击 Next 完成安装
...
前往 [code.visualstudio.com](https://code.visualstudio.com) 下载安装包。
...
推荐使用 Codex 或 Claude Code。这些 AI 工具内置了 Git 操作支持，可以用自然语言完成绝大部分 Git 操作。
...
如果你正在学习 Git 和 GitHub：

1. **马上动手**：创建一个测试仓库，用 VS Code + AI 工具实践一遍本文的所有操作
2. **参与开源**：找到一个你感兴趣的开源项目，尝试提交一个 Pull Request（哪怕只是修正一个拼写错误）
3. **建立习惯**：在自己的项目中实践小步提交、分支开发、PR Review 的完整工作流
4. **反复练习**：使用 [Git-Interactive-Tutorial](https://woyeyao.github.io/Git-Interactive-Tutorial/) 进行交互式操作训练
