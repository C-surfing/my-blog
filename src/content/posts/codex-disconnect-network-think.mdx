...
比赛环境里 `docker pull` 一个镜像能卡到怀疑人生，modelscope 下载模型权重也是几十 KB/s 的龟速。当时的解法很朴素：换国内镜像源、找加速镜像，或者是更换到实验室网络，换了可能就能用，但不理解为什么。

### 1.2 场景 B：Codex 桌面版 的 5/5 重连失败

Codex 推理强度开到最高之后，经常跑着跑着就弹"5/5 重新连接失败"。第一直觉是"CoT 太长，服务器传回来的时候被截断了"——这个直觉是错的。
...
高推理 不制造网络故障，它只是让连接活得更久，更容易撞上**本来就存在**的网络故障。
